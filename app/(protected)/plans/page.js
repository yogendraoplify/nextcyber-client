...
export default function Pricing() {
  ...
  // Find the plan the user is currently on
  const currentUserPlan = plans.find(
    (plan) =>
      plan.name.split(" ").join("_").toUpperCase() === userSubscriptionType &&
      plan.billingCycle === userSubscriptionBillingCycle,
  );

  // Derive the label and disabled state for each plan's CTA button
  const getPlanCTA = (plan) => {
    ...
    if (isCurrentPlan) {
      return { label: "Currently Active", isActive: true, disabled: true };
    }

    // No active paid subscription — user is on free / no plan
    if (!currentUserPlan || currentUserPlan.price === 0) {
      if (plan.price === 0) {
        // The free plan itself when user has no paid sub
        return { label: "Currently Active", isActive: true, disabled: true };
      }
      return { label: "Upgrade Now", isActive: false, disabled: false };
    }

    // User has a paid plan — compare prices
    if (plan.price > currentUserPlan.price) {
      return { label: "Upgrade Now", isActive: false, disabled: false };
    }

    if (plan.price < currentUserPlan.price) {
      return { label: "Downgrade Now", isActive: false, disabled: false };
    }

    // Same price but different billing cycle — shouldn't normally hit this,
    // but treat as a switch option
    return { label: "Switch Plan", isActive: false, disabled: false };
  };
    ...
    <section className="bg-g-900">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 max-w-5xl mx-auto mt-5">
        {filteredPlans.map((plan) => {
          const { label, isActive, disabled } = getPlanCTA(plan);
          ...
          return (
            <div key={plan.id}>
              <div className="border border-g-500 rounded-[20px] p-5">
                ...
                <button
                  className={`w-full mt-10 leading-4 text-sm py-2 px-4 font-medium rounded-full border border-g-500 transition-colors
                    ${
                      isActive
                        ? "bg-gradient-to-b from-accent-color-1 to-primary text-white cursor-not-allowed"
                        : "bg-g-600 text-g-200 hover:text-white hover:bg-gradient-to-b hover:from-accent-color-1 hover:to-primary cursor-pointer"
                    }
                  `}
                  disabled={disabled}
                  onClick={() => {
