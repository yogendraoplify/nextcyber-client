import { getCompanyPlansApi, getMentorPlansApi, getStudentPlansApi } from "@/api/planApi";
...
export const asyncGetMentorPlans = (setError) => async (dispatch) => {
  try {
    const { data } = await getMentorPlansApi();
    dispatch(setPlans(data.plans));
  } catch (error) {
    setError?.(getErrorMessage(error));
    toast.error(getErrorMessage(error));
